...
import com.taixin.ffmpeg.player.annotations.AccessedByNative;
import com.taixin.ffmpeg.player.annotations.CalledByNative;
import com.taixin.ffmpeg.player.option.AvFormatOption;
import com.taixin.ffmpeg.player.pragma.DebugLog;
 ...
 */
public final class TXMediaPlayer extends SimpleMediaPlayer {
    private final static String TAG = TXMediaPlayer.class.getName();
     ...
     */
    private static TXLibLoader sLocalLibLoader = new TXLibLoader() {
        @Override
    ...
    public static void loadLibrariesOnce(TXLibLoader libLoader) {
        synchronized (TXMediaPlayer.class) {
            if (!mIsLibLoaded) {
                libLoader.loadLibrary("stlport_shared");
                libLoader.loadLibrary("txffmpeg");
                libLoader.loadLibrary("txutil");
                libLoader.loadLibrary("txadk");
                libLoader.loadLibrary("txsdl");
                libLoader.loadLibrary("txplayer");
                mIsLibLoaded = true;
            }
        }
    }
     ...
     * @param reserved
     */
    public TXMediaPlayer(TXLibLoader libLoader) {
        initPlayer(libLoader);
    }

    private void initPlayer(TXLibLoader libLoader) {
         ...
         */
        native_setup(new WeakReference<TXMediaPlayer>(this));
    }
    ...
    private static class EventHandler extends Handler {
        private WeakReference<TXMediaPlayer> mWeakPlayer;

        public EventHandler(TXMediaPlayer mp, Looper looper) {
            super(looper);
            mWeakPlayer = new WeakReference<TXMediaPlayer>(mp);
        }
    ...
    private static void postEventFromNative(Object weakThiz, int what,
        ...
        TXMediaPlayer mp = (TXMediaPlayer) ((WeakReference) weakThiz).get();
        if (mp == null) {
            return;
        }
    ...
    private static int onControlResolveSegmentCount(Object weakThiz) {
        ...
        WeakReference<TXMediaPlayer> weakPlayer = (WeakReference<TXMediaPlayer>) weakThiz;
        TXMediaPlayer player = weakPlayer.get();
        if (player == null)
            return -1;
