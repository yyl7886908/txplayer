...
#import "TXSDLGLShader.h"
#import "TXSDLGLRender.h"
#import "TXSDLGLRenderI420.h"
#import "TXSDLGLRenderRV24.h"
...
static NSString *const g_vertexShaderString = TX_SHADER_STRING
(
...
@interface TXSDLGLView()
@property(atomic,strong) NSRecursiveLock *glActiveLock;
@property(atomic) BOOL glActivePaused;
@end

@implementation TXSDLGLView {
    ...
    if (self) {
        ...
        switch (chroma) {
            case SDL_FCC_I420:
                ...
                _renderer = [[TXSDLGLRenderI420 alloc] init];
                _bytesPerPixel = 1;
                break;
            case SDL_FCC_RV24:
                NSLog(@"OK use RV24 GL renderer");
                _frameChroma = SDL_FCC_RV24;
                _renderer = [[TXSDLGLRenderRV24 alloc] init];
                _bytesPerPixel = 3;
                break;
            default:
...
- (void)display: (SDL_VoutOverlay *) overlay
{
    ...
    if (![self tryLockGLActive]) {
        NSLog(@"TXSDLGLView:display: unable to tryLock GL active\n");
        return;
    }
...
- (void)displayInternal: (SDL_VoutOverlay *) overlay
{
    if (![self setupDisplay:overlay]) {
        NSLog(@"TXSDLGLView: setupDisplay failed\n");
        return;
    }

    if (_context == nil) {
        NSLog(@"TXSDLGLView: nil EAGLContext\n");
        return;
    }
...
{
    NSLog(@"TXSDLGLView:applicationWillEnterForeground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationDidBecomeActive
{
    NSLog(@"TXSDLGLView:applicationDidBecomeActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:NO];
}

- (void)applicationWillResignActive
{
    NSLog(@"TXSDLGLView:applicationWillResignActive: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationDidEnterBackground
{
    NSLog(@"TXSDLGLView:applicationDidEnterBackground: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}

- (void)applicationWillTerminate
{
    NSLog(@"TXSDLGLView:applicationWillTerminate: %d", (int)[UIApplication sharedApplication].applicationState);
    [self toggleGLPaused:YES];
}
