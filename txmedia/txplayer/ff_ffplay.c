...
static int read_thread(void *arg)
{
    ...
    if (is->video_stream < 0 && is->audio_stream < 0) {
        av_log(NULL, AV_LOG_FATAL, "Failed to open file = %s, video_stream = %d, audio_stream = %d \n",
               is->filename, is->video_stream, is->audio_stream);
...
void ffp_check_buffering_l(FFPlayer *ffp)
{
    ...
    if (hwm_in_ms > 0) {
        ...
        if (video_cached_duration > 0 && audio_cached_duration > 0) {
            cached_duration_in_ms = (int)TXMIN(video_cached_duration, audio_cached_duration);
