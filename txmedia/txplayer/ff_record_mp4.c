...
void * rtsp2mp4(void * args)
{
    ...
    char *in_filename_v = rtsp_url;//输入文件名（Input file URL）  
    ...
    /* char *in_filename_a =rtsp_url;   */
  
    char *out_filename = out_file_name;//输出文件名（Output file URL）  
...
void start_recording_rtsp_stream(char *rtsp_stream, char *filename, int time)
{
    ...
    int ret=pthread_create(&record_thread,NULL,(void*)rtsp2mp4 ,NULL);
    /* rtsp2mp4(); */
    if(ret)
        {
