...
#include "txsdl_vout_overlay_ffmpeg.h"

#include <assert.h>
#include "../txsdl_stdinc.h"
#include "../txsdl_mutex.h"
#include "../txsdl_vout_internal.h"
#include "../txsdl_video.h"
#include "txsdl_inc_ffmpeg.h"
#include "txsdl_image_convert.h"
...
SDL_VoutOverlay *SDL_VoutFFmpeg_CreateOverlay(int width, int height, Uint32 format, SDL_Vout *display)
{
    ...
    case SDL_FCC_YV12: {
        ...
        buf_width = TXALIGN(width, 16); // 1 bytes per pixel for Y-plane
...
#else
        buf_width = TXALIGN(width, 16); // unknown platform
#endif
        opaque->planes = 3;
        break;
    }
    case SDL_FCC_RV16: {
        ...
        buf_width = TXALIGN(width, 8); // 2 bytes per pixel
        opaque->planes = 1;
        break;
    }
    case SDL_FCC_RV24: {
    ...
    case SDL_FCC_RV32: {
        ...
        buf_width = TXALIGN(width, 4); // 4 bytes per pixel
        opaque->planes = 1;
        break;
    }
    default:
    ...
    struct SwsContext **p_sws_ctx, int sws_flags)
{
    ...
    // swscale / direct draw
    /*
    ALOGE("tx_image_convert w=%d, h=%d, df=%d, dd=%d, dl=%d, sf=%d, sd=%d, sl=%d",
          (int)frame->width,
          (int)frame->height,
          (int)dst_format,
    ...
    if (use_linked_frame) {
        // do nothing
    } else if (tx_image_convert(frame->width, frame->height,
