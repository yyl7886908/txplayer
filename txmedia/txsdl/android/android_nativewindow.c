...
#include "../ffmpeg/txsdl_inc_ffmpeg.h"
#include "txsdl_inc_internal_android.h"
...
static int android_render_yv12_on_yv12(ANativeWindow_Buffer *out_buffer, const SDL_VoutOverlay *overlay)
{
    ...
    int min_height = TXMIN(out_buffer->height, overlay->h);
    int dst_y_stride = out_buffer->stride;
    int dst_c_stride = TXALIGN(out_buffer->stride / 2, 16);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (dst_line_size == src_line_size) {
            ...
            // TODO: 9 padding
            int bytewidth = TXMIN(dst_line_size, src_line_size);
...
int sdl_native_window_display_l(ANativeWindow *native_window, SDL_VoutOverlay *overlay)
{
    ...
    int buff_w = TXALIGN(overlay->w, 2);
    int buff_h = TXALIGN(overlay->h, 2);
